...
package agile.core
{
	...
	import agile.bundle.BundleInstallDescriptor;
	import agile.bundle.BundleService;
	import agile.bundle.Extension;
	import agile.core.IAgilePreloader;
	import agile.core.LaunchRunner;
	import agile.core.LauncherManifest;
	import agile.core.StartupEvent;
	...
	/**
	 * Launcher provides the main bootstrapping code for all Agile applications.  In a regular
	...
	public class Launcher
	{
		...
		private static var agilePreloader:IAgilePreloader;
		...
		public static function findPreloader():void
		{
			...
			agilePreloader = findAgilePreloader(systemPreloader);	
		}
		
		private static function findAgilePreloader(container:DisplayObjectContainer):IAgilePreloader
		{			
			for (var i:int = 0; i < container.numChildren; i++) 
			{
				if (container.getChildAt(i) is IAgilePreloader)
				{
					return container.getChildAt(i) as IAgilePreloader;
				}
				else
				{
					if (container.getChildAt(i) is DisplayObjectContainer)
					{
						var p:IAgilePreloader = findAgilePreloader(container.getChildAt(i) as DisplayObjectContainer);
						if (p != null)
							return p;
					}
				}
		...
		public static function launch(manifest:LauncherManifest):void
		{
			if (agilePreloader != null)
			{
				...
				preloaderParentComponent.addChild(agilePreloader as DisplayObject);
			...
			bundleService.agilePreloader = agilePreloader;
			agilePreloader = null;
		...
		{			
			...
			for (var i:int = 0; i < exts.length; i++) 
			{
				...
				listener.dispatchEvent(new StartupEvent(StartupEvent.AGILE_INITIALIZED));
			}
		...
		{					
			...
			if (bundleService.agilePreloader != null)
			{
				bundleService.agilePreloader.addEventListener(StartupEvent.PRELOADER_CLOSE_COMPLETE,onPreloaderComplete);
				bundleService.agilePreloader.dispatchEvent(new StartupEvent(StartupEvent.PRELOADER_CLOSE_START));
			}
		...
		private static function onPreloaderComplete(event:Event):void
		{
			...
			if (bundleService.agilePreloader != null)
			{
				...
				bundleService.agilePreloader = null;
				agilePreloader = null;
			}
		}
