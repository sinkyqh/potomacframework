...
package agile.ui
{
	...
	[ExtensionPointDetails(id="Template", description="Declares a new top-level Agile UI template")]
	...
	/**
	 * PotomaUI is the main controller for the Agile UI framework.  Its primarily responsible for
	 * managing pages.
	 */
	public class AgileUI extends EventDispatcher
	{
		...
		[Inject]
		/**
		 * Callers should not construct AgileUI. Its is available for injection.
		 */
		public function AgileUI(bundleSrv:IBundleService, pageFactory:PageFactory)
		{
			...
			addEventListener(AgileEvent.PART_SAVED, onPartSaveComplete);
			addEventListener(AgileEvent.PART_SAVE_ERROR, onPartSaveError);
			addEventListener(AgileEvent.PART_DIRTY_CHANGE, onPartDirtyChange);
			addEventListener(AgileEvent.PART_CLOSED, onPartClosed);
		}
		...
		public function closePage(page:Page, promptForSave:Boolean=true):void
		{
			...
			dispatchEvent(new AgileEvent(AgileEvent.PAGE_CLOSED, null, page));
		}
		...
		private function onPartSaveComplete(e:AgileEvent):void
		{
		...
		private function onPartDirtyChange(e:AgileEvent):void
		{
			_template.dispatchEvent(new TemplateEvent(TemplateEvent.PAGE_DIRTY_CHANGE, null, null, null, null, e.page, true));
		}

		private function onPartClosed(e:AgileEvent):void
		{
			_template.dispatchEvent(new TemplateEvent(TemplateEvent.PAGE_DIRTY_CHANGE, null, null, null, null, e.page, true));
		}

		private function onPartSaveError(e:AgileEvent):void
		{
			if (_pageVars[e.page] != null && _pageVars[e.page].saving == true)
			{
				var savingArray:Array=_pageVars[e.page].savingPageRefs as Array;
				savingArray.splice(savingArray.indexOf(e.partReference), 1);
		...
		private function completeSave(page:Page):void
		{
			...
			{
				dispatchEvent(new AgileEvent(AgileEvent.PAGE_SAVED, null, page));
			}
			else
			{
				dispatchEvent(new AgileEvent(AgileEvent.PAGE_SAVE_ERROR, null, page));
			}
