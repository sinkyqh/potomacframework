...
{ 
	...
	import agile.core.IAgilePreloader;
	import agile.core.Agile;
	import agile.inject.InjectionEvent;
	import agile.inject.InjectionRequest;
	...
	*
	* @eventType agile.bundle.BundleEvent.BUNDLE_READY
	*/
	[Event(name="bundleReady", type="agile.bundle.BundleEvent")]

	/**
	 * Dispatched when a bundle installation is initiated.
	 *
	 * @eventType agile.bundle.BundleEvent.BUNDLES_INSTALLING
	 */
	[Event(name="bundlesInstalling", type="agile.bundle.BundleEvent")]
	
	/**
	 * Dispatched when bundle preloading is initiated.
	 *
	 * @eventType agile.bundle.BundleEvent.BUNDLES_PRELOADING
	 */
	[Event(name="bundlesPreloading", type="agile.bundle.BundleEvent")]
	
	/**
	 * Dispatched when a bundle file (either the assets.swf or the bundle.swf) is being downloaded.
	 *
	 * @eventType agile.bundle.BundleEvent.BUNDLE_PROGRESS
	 */
	[Event(name="bundleProgress", type="agile.bundle.BundleEvent")]
	
	/**
	* Dispatched when a set of bundles is installed.
	*
	* @eventType agile.bundle.BundleEvent.BUNDLES_INSTALLED
	*/
	[Event(name="bundlesInstalled", type="agile.bundle.BundleEvent")]

	/**
	* Dispatched when one or more extensions are added or removed from the extension registry.
	*
	* @eventType agile.bundle.ExtensionEvent.EXTENSIONS_UPDATED
	*/
	[Event(name="extensionsUpdated", type="agile.bundle.ExtensionEvent")]
	
	/**
	 * Dispatched when an error is encountered during bundle installation or loading.
	 *
	 * @eventType agile.bundle.BundleEvent.BUNDLE_ERROR
	 */
	[Event(name="bundleError", type="agile.bundle.BundleEvent")]
	
	/**
	 * Dispatched when a bundle starts loading.
	 * 
	 * @eventType agile.bundle.BundleEvent.BUNDLE_LOADING
	 */
	[Event(name="bundleLoading",type="agile.bundle.BundleEvent")]
	 ...
	 * 
	 * @eventType agile.bundle.BundleEvent.PREDOWNLOAD
	 */
	[Event(name="predownload",type="agile.bundle.BundleEvent")]
	
	/**
	 * Dispatched after the bundle service completed downloading a resource or asset of
	 * a bundle.  Currently this is either an assets.swf or the main bundle.swf.
	 * 
	 * @eventType agile.bundle.BundleEvent.POSTDOWNLOAD
	 */
	[Event(name="postdownload",type="agile.bundle.BundleEvent")]
	...
	public class BundleService extends EventDispatcher implements IBundleService
	{
		 ...
		 * @private
		 */
		public var agilePreloader:IAgilePreloader;
		private var currentlyInstalling:Boolean = false;
		
		private var pendingPreloads:Array = new Array();
		 
		private static var logger:ILogger = Log.getLogger("agile.bundle.BundleService");
		...
		public function install(installables:Array):void
		{
			...
			if (agilePreloader != null)
				agilePreloader.dispatchEvent(new BundleEvent(BundleEvent.BUNDLES_INSTALLING));
...
//				loader.contentLoaderInfo.addEventListener(ProgressEvent.PROGRESS,onProgress);
		...
		private function onProgress(event:ProgressEvent):void
		{
			...
			if (agilePreloader != null)
				agilePreloader.dispatchEvent(bundleEvent);
		}
		...
		private function onAssetsBytesReady(e:Event):void
		{
			...
			var className:String = "agileAssets_" + id;
		...
		private function handleBundleXML(bundle:String,xml:XML):void
		{
			...
			if (bundleAssetCountdown == 0)
			{
				if (pendingPreloads.length > 0)
				{
					...
					if (agilePreloader != null)
						agilePreloader.dispatchEvent(new BundleEvent(BundleEvent.BUNDLES_PRELOADING));
		...
		private function finalizeInstall():void
		{
			...
			if (agilePreloader != null)
				agilePreloader.dispatchEvent(new BundleEvent(BundleEvent.BUNDLES_INSTALLED));
		 ...
		 * all extensions declared in the entire class hierarchy are returned.  Importantly, for agile to
		...
		{			
			...
			//agile_core is the only special RSL bootstrap bundle (it should never be loaded)
			if (id == "agile_core")
			{
		...
		private function loadModule(bundle:String, url:String):void
		{
			...
			dispatchEvent(event);
			if (agilePreloader != null)
				agilePreloader.dispatchEvent(event);
                ...
                if (!isBundleLoaded(bundleID) && (isModuleLoaded(bundleID) || isModuleDataLoaded(bundleID)))//potentially now satisfied
                {
                    ...
                    if (isReady)
                    {
						if (isModuleLoaded(bundleID))
						{
							...
							dispatchEvent(e);
							if (agilePreloader != null)
								agilePreloader.dispatchEvent(e);
