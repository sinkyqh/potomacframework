...
package agile.inject
{
	...
	*
	* @eventType agile.inject.InjectionEvent.INJECTION_READY
	*/
	[Event(name="instanceReady", type="agile.inject.InjectionEvent")]

	/**
	 * Dispatched when a injection into request is complete.
	 * 
	 * @eventType agile.inject.InjectionEvent.INJECTINTO_COMPLETE
	 */	
	[Event(name="injectIntoComplete",type="agile.inject.InjectionEvent")]
	
	/**
	 * An injection request represent the asynchronous process necessary for the 
	 * creation and injection of objects in the Agile bundle system.
	 * <p>
...
//			_injector.sendPostInjectionEvent(postInjectEvent);
			...
			removeAllListeners(InjectionEvent.INJECTINTO_COMPLETE);
		}
		...
		private function instanceReady(event:InjectionEvent):void
		{
			...
			dispatchEvent(event.clone());
			removeAllListeners(InjectionEvent.INSTANCE_READY);
		}
		 ...
		 * @private
		 */
		internal function removeAllListeners(type:String):void
		{
			var toRemove:Array = new Array().concat(_listeners);
			for (var i:int = 0; i < toRemove.length; i ++)
			{
				removeEventListener(type,toRemove[i]);
			}
		}
	}
}