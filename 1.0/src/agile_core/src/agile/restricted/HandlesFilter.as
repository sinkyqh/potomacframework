package agile.restricted
{
	...
	import agile.core.AgileDispatcher;
	...
	[ExtensionPointDetails(id="Handles",attribute="global",description="If true, attaches a listener to the global AgileDispatcher",order="3",defaultValue="false")]
	...
	public class HandlesFilter extends EventDispatcher
	{
		...
		private var _agileDispatcher:AgileDispatcher;
		private var _awaitingInitialization:Dictionary = new Dictionary();
		
		[Inject]
		public function HandlesFilter(bundleSrv:IBundleService,agileDispatcher:AgileDispatcher)
		{
			_bundleService = bundleSrv;
			_agileDispatcher = agileDispatcher;
			...
			for (var i:int = 0; i < handles.length; i++)
			{
				...
				if (handles[i].global != undefined && handles[i].global == true)
				{
					...
					if (Extension(handles[i]).method.arguments.length == 1)
					{
						...
						{
							var typeXML:XML = describeType(argClz);
							isEventHandler = typeXML.factory.extendsClass.(@type=="flash.events::Event").length() > 0;
						}
					}
					...
					if (isEventHandler)
					{
						...
						_agileDispatcher.addEventListener(handles[i].event,listener,false,priority,true);
					}
					else
					{
						_agileDispatcher.addListener(handles[i].event,injectee,handles[i].method.name,priority);	
					}
				}
				else
				{
