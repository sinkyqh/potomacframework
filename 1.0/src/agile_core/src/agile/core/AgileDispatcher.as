...
package agile.core
{
	...
	/**
	 * The agileDispatcher along with the <code>[Handles]</code> tag is the backbone 
	 * of the agile messaging system.  
	 * <p>
	 * agileDispatcher manages two types of messaging.  The first is the standard event
	 ...
	 * available via the #addListener and #dispatch methods.  The agileDispatcher calls the 
	 ...
	 * </p><p>
	 * Clients may inject the AgileDispatcher and call methods on it directly.  Alternatively, if a 
	 * class is only required to listen for an event it may use the <code>[Handles]</code> tag with the
	 * attribute <code>global="true"</code> and agile will wire the event listener automatically. 
	 * </p><p>
	 * The agileDispatcher only uses weak references.
	 * </p>
	 */
	public class AgileDispatcher extends EventDispatcher
	{
		
		/**
		 * Returns the global agileDispatcher instance.  
		 * <p>
		 ...
		 * agileDispatcher instance and so this method is not considered harmful.
		 * </p> 
		 */
		public static function getInstance():AgileDispatcher
		{
			return Injector.getInstance().getInstanceImmediate(AgileDispatcher) as AgileDispatcher;
		}
		...
		/**
		 * agileDispatcher should not be instantiated by clients.  It is available for 
		 * injection instead. 
		 */
		public function AgileDispatcher()
		{
		}
		 ...
		 * agile is unable to provide a method signature similar to #addEventListener.
		 * agileDispatcher uses weak references only by design.  This particular bug 
		 ...
		 * weak reference (an absolute requirement), agile requires a reference to the 
