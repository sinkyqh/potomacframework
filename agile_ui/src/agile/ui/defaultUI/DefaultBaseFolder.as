...
package agile.ui.defaultUI
{
	...
	import agile.ui.Folder;
	import agile.ui.PartEvent;
	import agile.ui.PartExtensionManager;
	import agile.ui.PartInput;
	import agile.ui.PartOptions;
	import agile.ui.PartReference;
	import agile.ui.AgileEvent;
	import agile.ui.AgileUI;
	import agile.ui.SelectionEvent;
	...
	public class DefaultBaseFolder extends Folder
	{
		...
		protected var _agileUI:AgileUI;

		/**
		 * Callers should not construct Folders.  Folders should be constructed by Page instances via the FolderFactory.
		 */
		public function DefaultBaseFolder(injector:Injector, partExtensionMgr:PartExtensionManager, selectionSrv:SelectionService, agileUI:AgileUI)
		{
			super();
			_injector=injector;
		...
		private function onBroadcastEvent(e:PartEvent):void
		{
			...
			_agileUI.dispatchEventToParts(e.eventToBroadcast);
		}
		...
		private function onDirtyChange(e:PartEvent):void
		{
			...
			onIconLabelChange(e);
			_agileUI.dispatchEvent(new AgileEvent(AgileEvent.PART_DIRTY_CHANGE, e.target.parent.partReference as PartReference, page));
		}
		...
		private function onSelectionChanged(e:PartEvent):void
		{
			...
			_agileUI.dispatchEventToParts(event);
		}
		...
		override public function savePart(reference:PartReference):void
		{
			...
			{
				_agileUI.dispatchEvent(new AgileEvent(AgileEvent.PART_SAVED, reference, page));
				return; //TODO Should we just return with out dispatching the save?
			}
		...
		private function finishSave(part:UIComponent, sendEvent:Boolean):void
		{
			...
			if (sendEvent)
				_agileUI.dispatchEvent(new AgileEvent(AgileEvent.PART_SAVED, busyCanvas.partReference as PartReference, page));
		...
		private function onSaveError(e:PartEvent):void
		{
			...
			_agileUI.dispatchEvent(new AgileEvent(AgileEvent.PART_SAVE_ERROR, e.target.parent.partReference as PartReference, page));
		...
		override public function closePart(reference:PartReference, promptForSave:Boolean=true):void
		{
			...
			_agileUI.dispatchEvent(new AgileEvent(AgileEvent.PART_CLOSED, busyCanvas.partReference, page));
		}
